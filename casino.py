...
from templates.Funzionamento_del_Poker import (
    create_cards,
    deck_creation,
    deck_shuffle,
    draw_card,
...
def initialize_deck():
    if not os.path.exists(deck_file):
        ...
        for seed in organized_deck:
            organized_deck[seed] = sorted(
                organized_deck[seed], key=lambda x: values_of_cards.index(x["value"])
            )
    ...
    if request.method == "POST":
        ...
        if data:
            ...
            return render_template(
                "texas_holdem_game.html", value_of_chips=value_of_chips
            )
        else:
...
def play():
    if request.method == "POST":
        ...
        user_cards, dealer_cards, community_cards = create_cards(
            card_images_folder, suits
        )
        print(user_cards, dealer_cards, community_cards)
        return render_template(
            "play.html",
...
def convert_to_chips():
    ...
    if data:
        ...
        if not amount_str.strip() or not chip_color.strip():
            error_message = (
                "Error: Please enter a valid amount and select a chip color."
            )
            return render_template(
                "cashier_operations.html",
                value_of_chips=value_of_chips,
...
def reconvert():
    ...
    if data:
        ...
        remaining_money = (
            data["remaining_money"] + total_money
        )  # Aggiorna remaining_money con il totale convertito
        ...
        data["remaining_money"] = (
            remaining_money  # Aggiorna remaining_money con il totale convertito
        )
